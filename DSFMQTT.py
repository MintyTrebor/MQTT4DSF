...
try:
    with open("/opt/dsf/sd/sys/DSFMQTT_Config.json") as config_file:
                ...
                if j_MsgName["MsgName"] == msgName:
                    ...
                    for j_Msgs in j_MsgName["Msgs"]:
                        ...
                        if len(s_TMP_Topic) > 0 and len(s_TMP_MsgText) > 0:
                            ...
                            client.publish(str(s_TMP_Topic), str(s_TMP_MsgText))

    except Exception as ex:
        print("Construct Sys Msg : " + str(ex))
...
def DSFMQTT_ErrHandlerT1(errMsg):
    global s_MachineName
    global RepStr_MachineName
...
#Handle Errors Thread 2
def DSFMQTT_ErrHandlerT2(errMsg):
    global s_MachineName
    global RepStr_MachineName
    if errMsg == "[Errno 32] Broken pipe":
        #DSF is down - 
...
# Function to send a standard MQTT msg
def sendMsg(msgTopic, msgText):
    global MQTT_SVR_Add
    global MQTT_SVR_Port
    # small delay to avoid msg trying to send before previous one has completed
    time.sleep(0.2)
    try:
        if len(msgTopic) > 0 and len(msgText) > 0:
            client.connect(MQTT_SVR_Add,MQTT_SVR_Port,5)
            client.publish(str(msgTopic), str(msgText))

    except Exception as ex:
        constructSystemMsg("SysMsg", str("Error in sendMSG: " + str(ex)))
...
def getInitialInfo():
    global s_MachineName
    global RepStr_MachineName
    global subscribe_connection
    
    try:
        #setup the connection with the api
        subscribe_connection = pydsfapi.SubscribeConnection(SubscriptionMode.FULL, debug=False)
        subscribe_connection.connect()
        
        # Get the complete model
        s_machine_model = subscribe_connection.get_serialized_machine_model()
        subscribe_connection.close()

        #define machine json
        j_machine_model = json.loads(s_machine_model)

        subscribe_connection = None
        ...
        return(DSFMQTT_ErrHandlerT1(str("Err In getInitialInfo: "+ str(ex))))


# function that does the main job of monitoring for updates from DSF and publishing the MQTT messages
def monitoring():
    global s_MachineName
    global RepStr_MachineName
    global subscribe_connection
    ...
    try:
        ...
        #setup the connection with the api
        subscribe_connection = pydsfapi.SubscribeConnection(SubscriptionMode.PATCH, str_TMP_Filter, debug=False)
        subscribe_connection.connect()

    except Exception as ex:
        return(DSFMQTT_ErrHandlerT1(str("Err In monitoring: "+ str(ex))))

    try:
        while True:
            while subscribe_connection.get_machine_model_patch():
                j_patch = subscribe_connection.get_machine_model_patch()
                    ...
                    if s_TMP_Enabled == "Y" and b_IsCMD == False:
                        for j_Msg in j_AllMsgs["Msgs"]:
                            ...
                            if b_Match_Found == True and b_SndMsg == True:
                                s_TMP_MsgText = s_TMP_MsgText.replace(str(RepStr_MachineName),str(s_MachineName))
                                sendMsg(str(s_TMP_Topic), str(s_TMP_MsgText))
                subscribe_connection.connect()
                continue
            else:
                # assume time out so reconnect
                constructSystemMsg("SysMsg", "Connection to DSF has timed-out. DSFMQTT will attempt auto-recovery")
                subscribe_connection.close()
                subscribe_connection = pydsfapi.SubscribeConnection(SubscriptionMode.PATCH, str_TMP_Filter, debug=False)
                subscribe_connection.connect()
                continue
...
def timedMonitoring():
    ...
    while True:
        try:
            #setup the connection with the api
            subscribe_connection2 = pydsfapi.SubscribeConnection(SubscriptionMode.FULL, debug=False)
            subscribe_connection2.connect()
            
            # Get the complete model
            s_machine_model2 = subscribe_connection2.get_serialized_machine_model()
            subscribe_connection2.close()

            #define machine json
            j_machine_model2 = json.loads(s_machine_model2)

            subscribe_connection2 = None
            s_machine_model2 = None
            ...
            return(DSFMQTT_ErrHandlerT2(str("Err In timedMonitoring: "+ str(ex))))
